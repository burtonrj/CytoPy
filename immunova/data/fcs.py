...
class Population(mongoengine.EmbeddedDocument):
    ...
    def match_clustering_method(self, method: str, params: dict, features: list, return_match: bool = False):
        clusters = list()
        for c in self.clustering:
            x = {k: v for k, v in c.parameters} == params
            y = method == c.method
            z = set(features) == set(c.features)
            if all([x, y, z]):
                if return_match:
                    clusters.append(c)
                    continue
    ...
    def pull_clusters(self, method: str, params: dict, features: list):
        clusters = self.match_clustering_method(method, params, features, return_match=True)
        if not clusters:
            raise ValueError(f'Error: no clusters exist for the given method and parameters')
        return clusters

    def delete_clusters(self, method: str, params: dict, features: list):
        clusters = self.match_clustering_method(method, params, features, return_match=True)
        if not clusters:
            raise ValueError(f'No clusters exist for the given method and parameters, deletion unnecessary')
        self.clustering = [c for c in self.clustering if c not in clusters]
