...
def dbscan_gate(data, x, y, min_pop_size, distance_nn, expected_populations, core_only, sample_n=40000,
                nn=10):
    """
    ...
    :param channels: list of channel names for gating (e.g. x and y axis names)
    ...
    :param sample_n: sample size to perform clustering on (due to computational complexity, N > 50000 is not
    recommended)
    ...
    """
    output = dict(pos_index=None, warnings=[], error=0, error_msg=None)
    data = data.copy()
    sampled = True
    if sample_n < data.shape[0]:
        s = data[[x, y]].sample(sample_n)
    else:
        s = data[[x, y]]
        sampled = False
    ...
    if len(set([x for x in db_labels if x != -1])) != len(expected_populations):
        output['warnings'].append(f'Expected {len(expected_populations)} populations, identified {len(np.where(db_labels != -1)[0])}')
    ...
    identified_populations = collections.defaultdict(list)
    for p in expected_populations:
        label = knn.predict(np.reshape(p['target'], (1, -1)))
        identified_populations[label[0]].append(p['id'])

    for l, p_id in identified_populations.items():
        if len(p_id) > 1:
    ...
    identified_populations[-1] = ['noise']

    def rename_label(x):
        if x in identified_populations.keys():
            return identified_populations[x][0]
        return 'noise'

    data['labels'] = data['labels'].apply(rename_label)

    output['pos_index'] = {x: data[data['labels'] == x].index.values for x in data['labels'].unique()}
    return output
