...
def density_1d_fmo(data: pd.DataFrame, fmo_x: pd.DataFrame,
                   x: str, kde_bw=0.01, kde_frac=0.5, q=0.99, peak_t=0.01, fmo_z=2):
    """
...
def density_2d_fmo(data, fmo_x, fmo_y, x, y, child_populations: dict,
    ...
    fmo_result_x = density_1d_fmo(data, fmo_x, x=x, kde_bw=kde_bw, q=q, peak_t=peak_t)
    fmo_result_y = density_1d_fmo(data, fmo_y, x=y, kde_bw=kde_bw, q=q, peak_t=peak_t)
    # Check for errors
    ...
    geom['threshold_x'] = fmo_result_x.child_populations[f'{x}+']['geom']['threshold']
    geom['threshold_y'] = fmo_result_y.child_populations[f'{y}+']['geom']['threshold']
    geom['method'] = fmo_result_x.child_populations[f'{x}+']['geom']['method'] + ' ' + \
                     fmo_result_y.child_populations[f'{y}+']['geom']['method']
    return multidem_density_output(x, y, fmo_result_x, fmo_result_y, output, geom)