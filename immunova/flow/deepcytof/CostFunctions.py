...
class MMD:
    def __init__(self, mmd_layer, mmd_target_train, mmd_target_validation_split=0.1,
                 mmd_target_sample_n=1000, n_neighbors = 20, scales = None, weights = None):
        if scales is None:
            print('setting scales using KNN')
            med = np.zeros(20)
            for ii in range(1, 20):
                sample = mmd_target_train[np.random.randint(mmd_target_train.shape[0], size=mmd_target_sample_n), :]
        ...
        self.MMDLayer = mmd_layer
        mmd_target_train, mmd_target_validation = train_test_split(mmd_target_train,
                                                                   test_size=mmd_target_validation_split,
                                                                   random_state=42)
        self.MMDTargetTrain = K.variable(value=mmd_target_train)
        self.MMDTargetTrainSize = K.eval(K.shape(self.MMDTargetTrain)[0])
        self.MMDTargetValidation = K.variable(value=mmd_target_validation)
        self.MMDTargetValidationSize = K.eval(K.shape(self.MMDTargetValidation)[0])
        self.MMDTargetSampleSize = mmd_target_sample_n
    ...
    def RaphyKernel(self, x, y):
        ...
        sqdist = K.expand_dims(squared_distance(x, y), 0)
    ...
    def KerasCost(self, y_true, y_pred):
        ...
        sample = K.cast(K.round(K.random_uniform(shape=tuple([self.MMDTargetSampleSize]), minval=0,
                                                 maxval=self.MMDTargetTrainSize - 1)), IntType)
        # this is a subset operation (not a very pretty way to do it)
        mmd_target_sample_train = K.gather(self.MMDTargetTrain, sample)
        # do the same for the validation set
        sample = K.cast(K.round(K.random_uniform(shape=tuple([self.MMDTargetSampleSize]), minval=0,
                                                 maxval=self.MMDTargetValidationSize - 1)), IntType)
        # and the subset operation
        mmd_target_sample_validation = K.gather(self.MMDTargetValidation, sample)
        # create the sample based on whether we are in training or validation steps
        mmd_target_sample = K.in_train_phase(mmd_target_sample_train, mmd_target_sample_validation)
        # return the MMD cost for this subset
        ret = self.cost(self.MMDLayer, mmd_target_sample)
