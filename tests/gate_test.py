...
def test_child_init():
    test_child = gate.Child(name="test", signature={"x": 2423, "y": 2232, "z": 4543})
...
def test_childthreshold_init():
    test_child = gate.ChildThreshold(
        name="test",
...
def test_transform_none():
    g = gate.Gate(gate_name="test", parent="test parent", x="X", y="Y", method="manual")
    data = pd.DataFrame(
        {
    ...
    for i in ["X", "Y"]:
        assert transformed[i].mean() == pytest.approx(1.0, 0.1)
...
def test_transform_x():
    ...
    assert transformed.shape[0] == 1000
    assert transformed.shape[1] == 2
    assert transformed["X"].mean() != pytest.approx(1.0, 0.1)
    assert transformed["X"].std() != pytest.approx(0.5, 0.1)
    assert transformed["Y"].mean() == pytest.approx(1.0, 0.1)
...
@pytest.mark.parametrize(
    "kwargs",
    [{"method": "uniform", "n": 500}, {"method": "faithful"}, {"method": "density"}],
)
...
def test_upsample():
    data, labels = make_blobs(n_samples=3000, n_features=2, centers=3, random_state=42)
    ...
    g = gate.Gate(
        ...
        y="Y",
        method="manual",
        sampling={"method": "uniform", "frac": 0.5},
    )
    sample = g._downsample(data=data)
...
def test_dim_reduction():
    g = gate.Gate(
        ...
        y="Y",
        method="manual",
        dim_reduction={"method": "UMAP", "n_neighbors": 100},
    )
    data = pd.DataFrame(
        {
...
def test_threshold_fit_2d(yeo_johnson):
    data, labels = make_blobs(
        n_samples=3000,
        n_features=2,
        centers=[(1.0, 1.0), (1.0, 5.0), (5.0, 0.2)],
        random_state=42,
    )
...
def test_threshold_fit_predict_2d(yeo_johnson):
    data, _ = make_blobs(
        n_samples=4000,
        n_features=2,
        centers=[(1.0, 1.0), (1.0, 7.0), (7.0, 2.0), (7.0, 6.2)],
        random_state=42,
    )
    data = pd.DataFrame({"X": data[:, 0], "Y": data[:, 1]})
    method_kwargs = {"yeo_johnson": yeo_johnson}
    ...
    data, _ = make_blobs(
        n_samples=3000,
        n_features=2,
        centers=[(1.0, 1.0), (1.0, 7.0), (7.0, 6.2)],
        random_state=42,
    )
    data = pd.DataFrame({"X": data[:, 0], "Y": data[:, 1]})
...
def create_polygon_gate(klass, method: str, **kwargs):
    g = klass(
        gate_name="test",
        parent="test parent",
        x="X",
...
def test_polygon_add_child():
    ...
    g.add_child(
        gate.ChildPolygon(
            name="test",
            geom=PolygonGeom(
                x_values=np.linspace(0, 1000, 1).tolist(),
...
def test_polygon_generate_populations():
    data, labels = make_blobs(
        ...
        n_features=2,
        cluster_std=0.5,
        centers=[(1.0, 1.0), (1.0, 7.0), (7.0, 6.2), (6.0, 1.0)],
        random_state=42,
    )
    data = pd.DataFrame(data, columns=["X", "Y"])
    g = create_polygon_gate(klass=gate.PolygonGate, method="MiniBatchKMeans")
    polys = [
        Polygon([(-1.0, -1), (-1, 10), (3, 10), (3, -1), (-1, -1)]),
        Polygon([(4, -1), (8, -1), (8, 3.8), (4, 3.8), (4, -1)]),
        Polygon([(4, 4), (4, 10), (10, 10), (10, 4), (4, 4)]),
    ]
    pops = g._generate_populations(data=data, polygons=polys)
    assert len(pops) == 3
    assert all([isinstance(p, gate.Population) for p in pops])
...
def test_polygon_match_to_children():
    data, labels = make_blobs(
        ...
        n_features=2,
        cluster_std=1,
        centers=[(1.0, 1.0), (10.0, 6.2), (1.5, 2.0), (11, 7.0), (11.5, 7.5)],
        random_state=42,
    )
    data_dict = [
        {
    ...
    for i in [0, 1]:
        g.add_child(
            gate.ChildPolygon(
                ...
                geom=PolygonGeom(
                    x_values=data_dict[i].get("poly").exterior.xy[0],
                    y_values=data_dict[i].get("poly").exterior.xy[1],
                ),
            )
        )
    pops = g._generate_populations(
        data=pd.DataFrame(data, columns=["X", "Y"]),
...
def test_polygon_fit(gate):
    ...
    centroids = [
        create_envelope(
            data.loc[np.where(labels == i)]["X"].values,
            data.loc[np.where(labels == i)]["Y"].values,
        ).centroid.xy
