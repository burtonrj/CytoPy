...
@pytest.mark.parametrize(
    "query,expected,regex,permutations,case",
    [
...
def test_query_normalised_name_list():
    ref = [
        NormalisedName(standard="CD4", regex_str=r"^\s*CD[\-\s]*4\s*$"),
        NormalisedName(standard="HLA-DR", regex_str=r"^\s*HLA[\-\s]*DR\s*$"),
        NormalisedName(
            standard="CD45",
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"channel": "  FITC  ", "marker": "CD4-FITC"},
            {"channel": "FITC", "marker": "CD4"},
        ),
        ({"channel": "FSC-A", "marker": ""}, {"channel": "FSC-A", "marker": None}),
        (
...
def test_standardise_names(example, expected):
    ...
    ref_markers = [
        ...
        NormalisedName(standard="CD3", regex_str=r"\s*CD3\s*"),
    ]
    ref_mappings = [
        ChannelMap(channel="FITC", marker="CD4"),
        ChannelMap(channel="FSC-A"),
        ChannelMap(channel="APC-Fire-750", marker="CD3"),
    ]
    standardised = standardise_names(
        example,
...
def test_panel_create_from_excel():
    ...
    assert all(
        x in [cm.marker for cm in test.mappings]
        for x in [
            "FS Lin",
            "SS Log",
            "IgG1-FITC",
    ...
    assert all(
        x in [cm.channel for cm in test.mappings]
        for x in [
            "FS Lin",
            "SS Log",
            "FL1 Log",
    ...
    assert all(
        x in [m.standard for m in test.markers]
        for x in [
            "FS Lin",
            "SS Log",
            "IgG1-FITC",
    ...
    assert all(
        x in [c.standard for c in test.channels]
        for x in [
            "FS Lin",
            "SS Log",
            "FL1 Log",
...
def assert_h5_setup_correctly(filepath: str, exp: Experiment):
...
def test_load_data(example_populated_experiment):
    ...
    assert np.array_equal(
        data[data.sample_id == "test sample"]["original_index"].values,
        test_sample_pop1.index,
    )
    assert np.array_equal(
        data[data.sample_id == "test sample 2"]["original_index"].values,
