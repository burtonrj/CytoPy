...
class MissingExperimentError(DataError):
    def __init__(self, experiment_name: str):
        super().__init__(f"Invalid experiment, {experiment_name} does not exist!")


class DuplicateExperimentError(Exception):
    def __init__(self, experiment_name: str):
        super().__init__(f"Invalid experiment, {experiment_name} already exists!")


class DuplicateSubjectError(Exception):
    def __init__(self, subject_id: str):
        super().__init__(f"Invalid subject, {subject_id} already exists!")


class DuplicateSampleError(Exception):
    def __init__(self, sample_id: str):
        super().__init__(f"Invalid sample ID, {sample_id} already exists!")
...
class InvalidDataDirectory(Exception):
    def __init__(self, path: str):
        super().__init__(f"Invalid directory, {path} does not exists!")


class MissingSubjectError(Exception):
    def __init__(self, subject_id: str):
        super().__init__(f"Invalid subject, {subject_id} does not exists!")


class MissingSampleError(Exception):
    def __init__(self, sample_id: str):
        super().__init__(f"Invalid FileGroup, {sample_id} does not exists!")


class MissingControlError(Exception):
    def __init__(self, ctrl: str):
        super().__init__(f"Invalid control, {ctrl} does not exists!")


class MissingPopulationError(Exception):
    def __init__(self, population_id: str):
        super().__init__(f"Invalid population, {population_id} does not exists!")
