...
@pytest.mark.parametrize(
    "gate,child_n",
    [(create_threshold_gate, 4), (create_poly_gate, 1), (create_ellipse_gate, 3)],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            ["root", "Top right", "Top left", "Bottom populations"],
        ),
...
def test_apply_gate(example_populated_experiment, gate, populations):
    ...
    gate = gate()
    gate.fit(
        data=gs.filegroup.load_population_df(
    ...
    gs.apply_gate(gate=gate, plot=True)
    plt.show()
...
def test_add_hyperparameter_grid_invalid_gate(example_populated_experiment):
    ...
    with pytest.raises(AssertionError) as err:
        gs.add_hyperparameter_grid("invalid", params={})
...
def test_plot_backgate(example_populated_experiment):
    ...
    gs.plot_backgate(
        ...
        x="FSC-A",
        y="CD3",
        create_plot_kwargs={"transform_x": None, "transform_y": "logicle"},
    )
    plt.show()
...
def test_plot_population(example_populated_experiment):
    ...
    gs = apply_some_gates(gs)
    plt.close("all")
    gs.plot_population(population="pop1", x="FS Lin", y="IgG1-FITC")
    plt.show()
...
def test_edit_polygon_gate(example_populated_experiment):
    ...
    gs.edit_gate(
        "test ellipse",
        coords={"pop2": [[0, 1200, 1200, 0, 0], [-100, -100, 10000, 10000, -100]]},
    )
...
def test_delete(example_populated_experiment, remove_associations):
    ...
    gs.delete(remove_associations=remove_associations, delete_gates=remove_associations)
