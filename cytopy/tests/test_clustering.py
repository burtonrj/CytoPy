...
def multi_sample_data(example_populated_experiment):
    return load_population_data_from_experiment(
        experiment=multisample_experiment(example_populated_experiment),
        population="root",
        transform="logicle",
        verbose=True,
    )
...
def test_load_data(example_populated_experiment):
    data = load_population_data_from_experiment(
        experiment=multisample_experiment(example_populated_experiment),
        population="root",
        transform="logicle",
        verbose=True,
...
def test_sklearn_metaclustering(example_populated_experiment):
    data = multi_sample_data(example_populated_experiment)
    data["cluster_label"], data["meta_label"] = None, None
    clustered, _, _ = sklearn_clustering(
        data=data,
        features=FEATURES,
        verbose=True,
        method="MiniBatchKMeans",
    ...
    for _id, df in c.data.groupby("sample_id"):
        ...
        for cluster_id in df.meta_label.unique():
            ...
            with h5py.File(fg.h5path, "r") as f:
                ...
                assert len(f[f"index/{cluster_id}/primary"]) == cluster_sizes.get(
                    int(cluster_id.replace("cluster_", ""))
                )
